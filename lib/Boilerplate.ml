...
and map_anon_choice_exp_f079e30 (env : env) (x : CST.anon_choice_exp_f079e30) =
  (match x with
  | `Exp x -> R.Case ("Exp",
      map_expression env x
    )
...
and map_argument_list (env : env) ((v1, v2, v3) : CST.argument_list) =
  ...
  let v2 =
    (match v2 with
    | Some (v1, v2) -> R.Option (Some (
        let v1 = map_anon_choice_exp_f079e30 env v1 in
        let v2 =
          R.List (List.map (fun (v1, v2) ->
            let v1 = (* "," *) token env v1 in
            let v2 = map_anon_choice_exp_f079e30 env v2 in
            R.Tuple [v1; v2]
          ) v2)
        in
        R.Tuple [v1; v2]
...
and map_expression_not_binary (env : env) (x : CST.expression_not_binary) =
  ...
  | `Gnu_asm_exp (v1, v2, v3, v4, v5, v6) -> R.Case ("Gnu_asm_exp",
      let v1 =
        (match v1 with
        | `Asm tok -> R.Case ("Asm",
            (* "asm" *) token env tok
...
and map_type_definition (env : env) ((v1, v2, v3, v4, v5, v6, v7) : CST.type_definition) =
